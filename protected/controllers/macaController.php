...
class macaController extends Controller
{
	/**
	...
	function actionGetEstadoCivil2($criterio){
		...
		select pv_cliente.estado_civil as estado, 
			count (pv_cliente.estado_civil) as cantidad 
		FROM
			public.ss_predio_sistema_macadamia, 
			public.ss_predio, 
			public.pv_cliente,
			public.ss_estado
		WHERE 
			pv_cliente.estado_civil =ss_estado.estado AND
			ss_predio_sistema_macadamia.predio = ss_predio.codigo_predio AND
			ss_predio.codigo_cliente = pv_cliente.codigo_cliente'.$criterio.'
		Group by 
	...
	function actionGetTodoGenero($criterio){
		...
		from ss_genero left join(
			...
			FROM 
				...
				public.pv_cliente
			WHERE 
				ss_predio_sistema_macadamia.predio = ss_predio.codigo_predio
	...
	function actionGetProduccion($criterio){
		...
		foreach($mbd->query('SELECT 
			avg(produccion_lbs_ta) as lbstm,
			sum(ingreso_cultivos) as ing_cul,
			sum(ingreso_dop) as ing_dop
		...
		$b = null;
			...
			$b['lbstm'] = (float) $fila['lbstm'];
			$b['ing_cul'] = (float) $fila['ing_cul'];
			$b['ing_dop'] = (float) $fila['ing_dop'];
	...
	function actionGetCondicion($criterio){
		...
		foreach($mbd->query('SELECT 
			count (distinct ss_predio_sistema_macadamia.predio) as cantidad,
			calificacion as estado
		FROM 
			public.ss_predio_sistema_macadamia, 
			public.ss_predio,
			public. pv_cliente	
		WHERE 
			ss_predio_sistema_macadamia.predio = ss_predio.codigo_predio AND
			ss_predio.codigo_cliente = pv_cliente.codigo_cliente '.$criterio.'
		GROUP BY ss_predio_sistema_macadamia.calificacion'
			) as $fila) {
	...
	function actionGetRiesgo($criterio){
		...
		foreach($mbd->query('SELECT 
			count (distinct ss_predio_sistema_macadamia.predio) as cantidad,
			nivel_de_riesgo as estado,
			medidas_previstas as medidas
		FROM 
			public.ss_predio_sistema_macadamia, 
			public.ss_predio,
			public. pv_cliente	
	...
	function actionGetCapacitacion($criterio){
		...
		foreach($mbd->query('SELECT 
			tipo_capacitacion as estado,
			count (  distinct ss_predio_sistema_macadamia.predio) as cantidad,
					
			sum (tareas_plantadas) as tareas
		FROM 
			public.ss_predio_sistema_macadamia, 
			public.ss_predio,
			public. pv_cliente	
	...
	function actionGetCalificacion($criterio,$variedad){
		
		$mbd = new PDO('pgsql:host=localhost;port=5432;dbname=jbsoft', 'postgres', 'postgres');
		$rs = array();
		                 

		foreach($mbd->query('SELECT nombre_var'.$variedad.' as estado,
			
			
			sum (ss_predio_sistema_macadamia.ptas_plantadas_var'.$variedad.') as cantidad
		...
		WHERE 
			ss_predio_sistema_macadamia.predio = ss_predio.codigo_predio AND
			ss_predio_sistema_macadamia.nombre_var'.$variedad.' <>\'\' and
			ss_predio.codigo_cliente = pv_cliente.codigo_cliente'.$criterio.'
		Group by 
			ss_predio_sistema_macadamia.nombre_var'.$variedad.' 
		order by 	
		ss_predio_sistema_macadamia.nombre_var'.$variedad.'  ASC') as $fila) {
	...
	function actionGetSumaCalificacion($criterio){
		...
		foreach($mbd->query('SELECT
			sum (ss_predio_sistema_macadamia.ptas_plantadas_var2) +
			sum (ss_predio_sistema_macadamia.ptas_plantadas_var3) +
			sum (ss_predio_sistema_macadamia.ptas_plantadas_var4) +
			sum (ss_predio_sistema_macadamia.ptas_plantadas_var1) as cantidad
	...
	function actionGetSistemasSubcuenca($criterio){
		...
		foreach($mbd->query('SELECT
			 ...
			 sum(ss_predio_sistema_macadamia.tareas_plantadas) as tareas
	...
	function actionGetAvances($criterio){
		...
		FROM 
			...
			public.ss_predio
		WHERE 
			ss_predio_sistema_macadamia.predio = ss_predio.codigo_predio' .$criterio.' ') as $fila) {
...
FROM 
 ...
 Order by sf_descripcion_componentes.orden ASC;') as $fila) {
	...
	function actionGetResumen($criterio){
		...
		SELECT \'Monto financiado\' as concepto, sum(monto_financiado) as valor
	...
	Union
		...
		SELECT \'Participantes\' as concepto, count (distinct ss_predio_sistema_macadamia.predio ) as valor
	...
	Union
		...
		SELECT \'Asociados\' as concepto, count (distinct ss_predio_sistema_macadamia.predio ) as valor
		...
		WHERE   public.ss_predio_sistema_macadamia.tiene_asociacion = \'Si\' and
			ss_predio_sistema_macadamia.predio = ss_predio.codigo_predio AND
			
			ss_predio.codigo_cliente = pv_cliente.codigo_cliente' .$criterio.'
Union

		SELECT \'Reciben financiamiento \' as concepto, count (distinct ss_predio_sistema_macadamia.predio ) as valor
		...
		WHERE   public.ss_predio_sistema_macadamia.financiamiento = TRUE and
			ss_predio_sistema_macadamia.predio = ss_predio.codigo_predio AND
...
Union
		...
		SELECT \'Plantas sobrevivientes\' as concepto, sum ( ss_predio_sistema_macadamia.plantas_sobrevivientes ) as valor
		...
		$b = null;
			$b['descripcion'] =$fila['concepto'];
			$b['cantidad'] = (float)$fila['valor'];
 ...
 Order by sf_descripcion_componentes.orden
  ;') as $fila) {
	...
	function actionGetMujeres($criterio){
		...
		foreach($mbd->query('SELECT
			ss_predio_sistema_macadamia.mujeres_trabajando as actividad ,
			 count (id) as cantidad
		...
		Group by 
			ss_predio_sistema_macadamia.mujeres_trabajando

		Order by
			ss_predio_sistema_macadamia.mujeres_trabajando ') as $fila) {
	...
	function actionGetObreros($criterio){
		...
		foreach($mbd->query('SELECT
			ss_predio_sistema_macadamia.obreros_contratados as actividad ,
			 count (id) as cantidad
		...
		Group by 
			ss_predio_sistema_macadamia.obreros_contratados

		Order by
			ss_predio_sistema_macadamia.obreros_contratados ') as $fila) {
