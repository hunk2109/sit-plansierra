...
$col[] = array('text' => number_format($pobExtr*100/$pobTotal , 0, ",", "."), 'width' => '30', 'height' => '5', 'align' => 'C', 'font_name' => 'Arial', 'font_size' => '8', 'font_style' => '', 'fillcolor' => '224,224,224', 'textcolor' => '0,0,0', 'drawcolor' => '0,0,0', 'linewidth' => '0.4', 'linearea' => 'LTBR');  
$col[] = array('text' => number_format($totalExtranjerosEspana*100/$pobEspana , 0, ",", "."), 'width' => '30', 'height' => '5', 'align' => 'C', 'font_name' => 'Arial', 'font_size' => '8', 'font_style' => '', 'fillcolor' => '224,224,224', 'textcolor' => '0,0,0', 'drawcolor' => '0,0,0', 'linewidth' => '0.4', 'linearea' => 'LTBR');  
$columns[] = $col;
...
if($cifra_venta ){
	...
	$Criteria = new CDbCriteria();
	$Criteria->condition = "cod_zipcode = ".$_GET['cod_zipcode'];
	$mOld = $model->find($Criteria);
	$fecha_ini = $mOld->fecha_ini;
	...
	foreach($cod as $c){
		...
		if($k<11){
			...
			array_push($cifra, number_format($c->venta_si*100/$CV,1));
			array_push($cps, $c->cp);
		}else{
	...
	array_push($cifra, number_format($suma*100/$CV, 1));
	array_push($folletos, $sumFollSI*100/$sumFoll);
	
	//ZIPCODE ANTERIOR
	$model=new TblZc;
	$Criteria = new CDbCriteria();
	$Criteria->condition = "id_alcampo = ".$_GET['hiper'] . " AND fecha_ini < '".$fecha_ini."'";
	$Criteria->order = "fecha_ini";
	$old = $model->find($Criteria);
	if($old){
		$zipcodeAnterior = $old->cod_zipcode;
		$new_euros = $old->cv;
		
		$model=new TblZcConsolidado;
		$Criteria = new CDbCriteria();
		$Criteria->select = "sum(venta_si + venta_no + venta_ns) as venta_si";
		$Criteria->condition = "cod_zipcode = ".$zipcodeAnterior;
		$Criteria->group = "cod_zipcode";
		$cifra_venta_old = $model->find($Criteria);
		if($cifra_venta_old ){
			$CV_old = $cifra_venta_old->venta_si;
			$k = 0;
			$suma = 0;
			$suma_old = 0;				
			foreach($cps as $c){
				...
				$Criteria->condition = "cod_zipcode = ".$zipcodeAnterior . " AND cp = '".$c."'" ;
				$cod_old = $model->find($Criteria);
				if($cod_old){
					$valor_euros = $new_euros*($cod_old->venta_si)/$CV_old;
					...
					//array_push($cifraOld, number_format($cod_old->venta_si*100/$CV_old, 1));
				}
				$k++;	
				
			}
			array_push($cifraOld, 100*($suma_old-$suma)/$euros);
		}
	}
 ...
 $MyData = new pData();  
 $MyData->addPoints($cifra,"Cifra de venta"); 
 //$MyData->addPoints($cifra,"CV"); 
 //$MyData->setSerieTicks("CV",4); 
