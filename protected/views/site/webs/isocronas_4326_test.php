...
var sel, selLayer, osm, google, google_carto,parcelassel,sal,salid,codigobuscado;
...
plantaciones_puntos = new ol.layer.Image({
	...
	url: 'http://sit.plansierra.org/cgi-bin/mapserv.exe?MAP=c:\\datos/sierra/raster.map&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetLegendGraphic&FORMAT=image/png&SLD_VERSION=1.1.0&LAYER=PLANTACIONES_PUNTOS',
	source: new ol.source.ImageWMS({
			...
			$.get( "index.php?r=puntos/GetPointsIntersect&x="+sal[0]+"&y="+sal[1]+"&resolution="+resolution, function( data ) {
				...
				if(data.length >1){
						...
						for(var j=0; j < data.length; j++){
							...
							$("<option />", {value: datos[j].gid, text:(datos[j].codigo +'  '+datos[j].tipo  +'  '+datos[j].especie  ) }).appendTo($select);
