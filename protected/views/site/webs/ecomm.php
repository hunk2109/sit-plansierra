...
$(document).ready(function () {
		...
		change: function( event, data ) {
			...
			if(idAlcampo != 65){
				$.get('index.php?r=TblEcom/GetPeriodos&id_hiper='+idAlcampo, function(data2){
					...
					cp_ecom.getSource().setUrl("http://<?php echo Yii::app()->params['servidor'];?>/cgi-bin/mapserv.exe?MAP=c:\\datos/sigma/maps/sigma.map&hiper="+idAlcampo+"&fecha="+formatDate(idEncuesta));
	...
	cp_ecom = new ol.layer.Image({
		...
		url: 'http://<?php echo Yii::app()->params['servidor'];?>/cgi-bin/mapserv.exe?MAP=c:\\datos/sigma/maps/sigma.map&alcampo=1&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetLegendGraphic&FORMAT=image/png&SLD_VERSION=1.1.0&LAYER=cp_ecom2016',
		source: new ol.source.ImageWMS({
		url: 'http://<?php echo Yii::app()->params['servidor'];?>/cgi-bin/mapserv.exe?MAP=c:\\datos/sigma/maps/sigma.map&alcampo=1',
		crossorigin: 'anonymous',
			params: {'LAYERS': 'cp_ecom2016', 'SRS': "EPSG:25830" }
		})
	})
