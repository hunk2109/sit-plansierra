					...
					<table id="Grafico_1_1" class="table table-bordered" width="100%">
						<thead>
							<tr>
								...
								<th>Plantas</th>
								<th>Tareas</th>
								<th>Inv. en plantas</th>
								<th>Inv. en labores</th>
...
<div id="Unidad" class="tabcontent">
  ...
  <br/>
		<div class="row">
			<button type="button" class="btn btn-primay" title="Calcular" onclick="cargarTablaFactura()">Buscar 
			...
			<div id="popup-content" style="font-size:20px">
				<div id="tabs">
				  <ul>
					...
					<li><a href="#tabs-A" style="color: FireBrick">Informe</a></li><br><br>
	...
	map = new ol.Map({		
		...
		layers: [
			...
			plantaciones_cuencas_selected = new ol.layer.Image({
				...
				url: 'http://sit.plansierra.org/cgi-bin/mapserv.exe?MAP=c:\\datos/sierra/especies.map&parcelassel=0&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetLegendGraphic&FORMAT=image/png&SLD_VERSION=1.1.0&LAYER=SOLOLINEA_PLANTACION_SELECTED',
				source: new ol.source.ImageWMS({
				url: 'http://sit.plansierra.org/cgi-bin/mapserv.exe?MAP=c:\\datos/sierra/especies.map&parcelassel=0',
				crossorigin: 'anonymous',
					params: {'LAYERS': 'sololinea_plantacion_selected', 'SRS': "EPSG:3857" }
				})
			})
...
/*	
	...
	groupCapas3 = new ol.layer.Group({
		...
		name: 'Imagen Google',
		visible: false,
		layers: [google, microcuencas_selected]
	});
...
//$("#Grafico_1_1").html('<img id="loading-image" width=100 src="images/logo3_75_75.gif" alt="Loading..." />');
	...
	if (data3.length>100 ){
	alert ('Numero de registros devueltos ' +data3.length+' solo se mostraran los 100 primeros en pantalla, limite la busqueda. Excel los exportará todos' );
	}
	else {
	...
	*/	
		...
		$('#Grafico_1_1 tbody').on ('click', 'tr' , function() {
		  ...
		  //alert ('aqui estoy ' +seccion );
		  //alert ( ' segunda ' +seccion);
		  /*$.get ('http://sit.plansierra.org/cgi-bin/mapserv.exe?MAP=c:\\datos/sierra/especies.map&parcelassel=4892g&SLD_VERSION=1.1.0&LAYER=PLANTACION_SELECTED', function (data){
			  
				alert ('aqui estoy 2');
		  var parser =new ol.format.WMSCapabilities();
		  var result =parser.read(data);
		  var extent =result.Capability.layer[2].BoundingBox[0].extent;
	...
	if (data10.length < 1) {
		alert ('ATENCION: No hay poligono digitalizado correcto para esta parcela');}
	//alert ('parcelas digitalizadas = ' +data10.length);
...
*/
	...
	// plantaciones_cuencas_selected tiene que estar definido como un layer
		/*	
			...
			$.get("http://sit.plansierra.org/cgi-bin/mapserv.exe?MAP=c:\\datos/sierra/especies.map&parcelassel="+seccion);
	...
	*/
		...
		for (var i=0; i < records; i++){
			...
			data3[i].factura,
			data3[i].plantas,
			"<td>" +data3[i].tareas.toLocaleString('en-EN', {minimumFractionDigits:2})+ "</td>",
			data3[i].inversion_plantas.toLocaleString('en-EN', {minimumFractionDigits:2}),
			data3[i].labores.toLocaleString('en-EN', {minimumFractionDigits:2}),
			(data3[i].inversion_plantas +data3[i].labores).toLocaleString('en-EN', {minimumFractionDigits:2})
...
//
//
//AQUI escribe la informacion sacarda de pantalla en la ventana del SUR
//
//
//
//
...
function verGrafico(data){
	...
	//3 TAB-4-	
				$('#tabs-4').empty();
				//alert ('estoy en tecnico codigobuscadao ' +codigobuscado);
	$.get( "index.php?r=puntos/GetActividad&plantacion="+codigobuscado, function( data20 ) {
				...
				var datosplantacion=[];
				//Cabecera
					//alert ('estoy aqui dentro de tecnico tab4 1');
				//alert ('data length ' +data20.length);
			for(var j=0; j < data20.length; j++){	
			   $.each(data20[j], function(i, item) {
				...
				//alert ('estoy aqui dentro de tecnico tab4 2');
			}
			$('<tr>').appendTo('#tabs-4');		
