	...
	$(document).ready(function () {
		...
		$('#hiperAlcampo').selectmenu({
			change: function( event, data ) {
				...
				cp_clientes.getSource().setUrl("http://<?php echo Yii::app()->params['servidor'];?>/cgi-bin/mapserv.exe?MAP=c:\\datos/sigma/maps/sigma.map&hiper="+idAlcampo);
		...
		cp_clientes = new ol.layer.Image({
			...
			url: 'http://<?php echo Yii::app()->params['servidor'];?>/cgi-bin/mapserv.exe?MAP=c:\\datos/sigma/maps/sigma.map&alcampo=1&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetLegendGraphic&FORMAT=image/png&SLD_VERSION=1.1.0&LAYER=cp_clientes',
			source: new ol.source.ImageWMS({
			url: 'http://<?php echo Yii::app()->params['servidor'];?>/cgi-bin/mapserv.exe?MAP=c:\\datos/sigma/maps/sigma.map&alcampo=1',
			crossorigin: 'anonymous',
				params: {'LAYERS': 'cp_clientes', 'SRS': "EPSG:25830" }
			})
		})
		...
		cp_clientes_seleccionar = new ol.layer.Image({
			...
			url: 'http://<?php echo Yii::app()->params['servidor'];?>/cgi-bin/mapserv.exe?MAP=c:\\datos/sigma/maps/sigma.map&alcampo=1',
			crossorigin: 'anonymous',
				params: {'LAYERS': 'cp_clientes_seleccionar', 'SRS': "EPSG:25830" }
			})
		})
				...
				$.get(url, function (data){
					...
					cpSelected.push("'"+cod+"'");
						selLayer = new ol.layer.Image({
							...
							crossorigin: 'anonymous',
								params: {'LAYERS': 'cp_clientes_select', 'SRS': "EPSG:25830" }
							})
						})
