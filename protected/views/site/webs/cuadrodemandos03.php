...
<div class="pane ui-layout-center">
	...
	<div style="width: 50%; height: 100%; float:right" >
		...
		<div class="col-md-10">
						...
						<div class="row">
								<div class="col-md-12">
									<div id="graficoIria" style="width:100%; height:200%"></div>
								</div>
								<div class="col-md-12">
									<div id="grafico1" style="width:100%; height:200%"></div>
								</div>
...
<div class="pane ui-layout-west">
	...
	<label>CUADRO DE MANDO 03</label>
...
var sel, selLayer, osm, google, google_carto,parcelassel,sal,salid,codigobuscado,solo_plantaciones,dataano,dataespecie, dataanocu, datacuenca, datamicrocuenca;
...
$.get('index.php?r=Graficos/GetPSsubMicroCuencas', function (data3){
	datamicrocuenca = JSON.parse(data3);
	
});	


$.get('index.php?r=Graficos/GetCuenMicrEspe', function (data4){
	dataespecie = JSON.parse(data4);
		Highcharts.setOptions({
		 colors: [
		 '#fffff','#3333aa99', '#0000ffcc', '#999999', '#bb0000bb',
		 '#000099', '#330000', '#00ff0099','#cccc3399', '#99339999',
		 '#33992299','#00ff00ff','#cccc33ff', '#993399ff', '#339922ff',
		'#33992299','#ffff4488','#cccc33aa', '#993399aa', '#339922aa'],
		lang: {
		  decimalPoint: '.',
		  thousandsSep: ''
		}
    ...
    for (i=0;i<datacuenca.length;i++) {
            ...
            regionP = {
               id: 'id_' + regionI,
			   name: datacuenca[i].cuenca,
               // color: Highcharts.getOptions().colors[regionI+1]
			}
			//colors: ['#ff0000','#0000ff','#ffffff','#ffffff','#ffffff', '#0000ff', '#990000', '#009900','#000099', '#330000', '#003300','#000033', '#000000'],
            countryI = 0;
            for (j=0;j<datamicrocuenca.length;j++) {
			 countryVal = 0;
                if (datacuenca[i].cuenca==datamicrocuenca[j].cuenca) {
					
                    countryP = {
                        id: regionP.id + '_' + countryI,
                        name: datamicrocuenca[j].microcuenca,
                        parent: regionP.id,
                    ...
                    for (l=0;l<dataespecie.length;l++){
						//alert (Highcharts.getOptions().colors[l]);
                        if (dataespecie[l].microcuenca == datamicrocuenca[j].microcuenca) {
							if (dataespecie[l].tareas> 100){
								//alert (l+'  '+dataespecie[l].especie+'   '+Highcharts.getOptions().colors[dataespecie[l].color]);
                              causeP = {
                                id: countryP.id + '_' + causeI,
							    name: dataespecie[l].especie,
								
                                parent: countryP.id, 
								color: Highcharts.getOptions().colors[dataespecie[l].color],
								//color: dataespecie[l].color,
								//color: '#'+Math.floor(Math.random()*16777215).toString(16),//Highcharts.getOptions().colors[l+j+i],

								//color:"'"+Highcharts.getOptions().colors[l]+"'",
																
								value: Math.round(dataespecie[l].tareas)
								
                              }
                              regionVal += causeP.value;
                              points.push(causeP);
							  countryVal += causeP.value;
                            }
							
				         }
						causeI = causeI + 1 
					}
					 //countryP.value= (countryVal);
           ...
           //regionP.value = (regionVal) ;
			//alert ('regionp value'+ regionP.value);
            points.push(regionP);
            regionI = regionI + 1;
	}		
	...
	console.info (JSON.stringify(points));
		...
		$('#grafico1').empty();
		$('#grafico1').highcharts({
	    colors: ['#ffffff','#ffffff', '#ffffff', '#999999', '#999999','#999999', '#ffffff', '#003300','#000033', '#000000'],
		/*chart: {
						// Edit chart spacing
						spacingBottom: 5,
						spacingTop: 5,
	...
	*/
	   ...
	   series: [{
        ...
        subtitle: {
            text: 'Especies con mas de 100 tareas'
        },
        title: {
            text: 'Especies por cuenca  microcuenca'
        }
    });
...
/*
    ...
    for (i=0;i<dataespecie.length;i++) {
            ...
            for (j=0;j<datamicrocuenca.length;j++) {
                ...
                if (dataespecie[i].microcuenca==datamicrocuenca[j].microcuenca) {
                    ...
                    for (l=0;l<datacuenca.length;l++){
                        ...
                        if (datacuenca[l].cuenca == datamicrocuenca[j].cuenca) {
							if (dataespecie[i].tareas> 100){
                              causeP = {
                                id: countryP.id + '_' + causeI,
							    name: datacuenca[l].cuenca,
                                parent: countryP.id, 
								//color: Highcharts.getOptions().colors[l+j+i],
								value: Math.round(dataespecie[i].tareas)
