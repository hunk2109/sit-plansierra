...
$(document).ready(function () {
		...
		change: function( event, data ) {
			...
			if(idAlcampo != 65){
				$.get('index.php?r=TblEcom/GetPeriodos&id_hiper='+idAlcampo, function(data2){
					...
					//cp_ecom.getSource().setUrl("http://<?php echo Yii::app()->params['servidor'];?>/cgi-bin/mapserv.exe?MAP=c:\\datos/sigma/maps/sigma_3857.map&hiper="+idAlcampo);
					cp_ecom.setVisible(true);
					cp_ecom.getSource().setUrl("http://<?php echo Yii::app()->params['servidor'];?>/cgi-bin/mapserv.exe?MAP=c:\\datos/sigma/maps/sigma_3857.map&hiper="+idAlcampo+"&fecha="+formatDate(idEncuesta));
	...
	*/
	...
	cp_ecom = new ol.layer.Image({
		...
		id: 'cp_ecom',
		visible: true,
		name: "Progresión E-commerce",
		url: 'http://<?php echo Yii::app()->params['servidor'];?>/cgi-bin/mapserv.exe?MAP=c:\\datos/sigma/maps/sigma_3857.map&alcampo=1&SERVICE=WMS&VERSION=1.3.0&REQUEST=GetLegendGraphic&FORMAT=image/png&SLD_VERSION=1.1.0&LAYER=cp_ecom2016',
		source: new ol.source.ImageWMS({
		url: 'http://<?php echo Yii::app()->params['servidor'];?>/cgi-bin/mapserv.exe?MAP=c:\\datos/sigma/maps/sigma_3857.map&alcampo=1',
		crossorigin: 'anonymous',
			params: {'LAYERS': 'cp_ecom2016', 'SRS': "EPSG:3857" }
		})
	})
	...
	cp_ecom_seleccionar = new ol.layer.Image({
		...
		url: 'http://<?php echo Yii::app()->params['servidor'];?>/cgi-bin/mapserv.exe?MAP=c:\\datos/sigma/maps/sigma_3857.map&alcampo=1',
		crossorigin: 'anonymous',
			params: {'LAYERS': 'cp_ecom_seleccionar', 'SRS': "EPSG:3857" }
		})
	})
	layers.push(cp_ecom);
	groupCapas = new ol.layer.Group({
		id:'ecom',
		name: 'Capas E-COMMERCE',
		layers: [cp_ecom]
	});
			...
			$.get(url, function (data){
			   ...
			   cpSelected.push("'"+cod+"'");
					selLayer = new ol.layer.Image({
						...
						crossorigin: 'anonymous',
							params: {'LAYERS': 'cp_ecom_select', 'SRS': "EPSG:3857" }
						})
					})
